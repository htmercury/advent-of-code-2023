...
def get_adj_nodes(moves_made, direction_idx, pos, city_lines, y_max, x_max):
    ...
    new_direction_idx = (direction_idx + 1) % len(directions)
    new_pos = move(pos, new_direction_idx)
    if is_valid(new_pos, y_max, x_max) and moves_made >= MIN_MOVES:
        adj_nodes.append((int(city_lines[new_pos[0]][new_pos[1]]), 1, new_direction_idx, new_pos))
...
def solution():
    ...
    src = (0, 0)
    root_node = (0, 0, 3, src)
    # needed due to ultra crucible can't turn until MIN_MOVES
    root_node_two = (0, 0, 2, src)
    dst = (y_max - 1, x_max - 1)

    pq = []
    heapq.heappush(pq, root_node)
    heapq.heappush(pq, root_node_two)

    distances = {}
    distances[root_node[1:]] = 0
    distances[root_node_two[1:]] = 0
    ...
    while len(pq) != 0:
        ...
        if pos == dst and moves_made >= MIN_MOVES:
            target = curr_state
            break
